@extends('dashboard.layouts.dashboard', ['title' => 'Clients'])
    ...
    <table class="table table-striped datatable-responsive datatable-header-basic" id="DataTables_Table_0">
        ...
        <tbody>
            @foreach ($clients as $client)
                <tr>
                    <td>{{ $client->libelle_ptn }}</td>
                    <td>{{ $client->telephone_ptn ?? '...' }}</td>
                    <td>{{ $client->created_at->format('d/m/Y à H:i:s') }}</td>
                    <td class="text-center">
                        <a href="{{ route('client.show', $client) }}" class="icon-eye8 text-brown-800 mr-2" data-toggle="tooltip" data-placement="bottom" title="Visualiser">
                        </a>
                        {{-- <a href="{{ route('client.edit', $client) }}" class="icon-pencil7 text-info" data-toggle="tooltip" data-placement="bottom" title="Modifier">
                        </a> --}}
                        <a href="{{ route('client.destroy', $client) }}" class="icon-trash text-danger" data-toggle="tooltip" data-placement="bottom" title="Supprimer" onclick="event.preventDefault(); confirmationModal('Voulez-vous continuer l\'action ?','delete-form-{{ $client->id_ptn }}');">
                            {{-- <i class="far fa-trash-alt"></i> --}}
                        </a>
                        <form id="delete-form-{{ $client->id_ptn }}" action="{{ route('client.destroy', $client) }}" method="POST" style="display: none;">
                            @csrf
                            @method('DELETE')
                        </form>
                    </td>
