@extends('dashboard.layouts.dashboard', ['title' => 'Modifier un projet'])
    ...
    <div class="breadcrumb-line breadcrumb-line-light header-elements-md-inline">
        <div class="d-flex">
            <div class="breadcrumb">
                ...
                <span class="breadcrumb-item">Modifier un projet</span>
                <span class="breadcrumb-item active">{{ $projet->libelle_prj }}</span>
            </div>
...
<div class="card p-4">
    <div class="row">
        <div class="col-xl-12 col-lg-12 col-sm-12">
            ...
            <form class="row" id="add" action="{{ route('projet.update', $projet) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                    ...
                    <div class="form-group mb-3">
                        ...
                        <select class="select @error('categorie') is-invalid @enderror" name="categorie" aria-describedby="categorie" placeholder="Sélectionner la categorie" required>
                            @foreach ($typeParametre->where('code_typ', 'cat')->first()->parametres as $parametre)
                                <option value="{{ $parametre->id_par }}" {{ $projet->idcat_prj == $parametre->id_par ? 'selected' : '' }}>{{ $parametre->libelle_par }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                    ...
                    <div class="form-group mb-3">
                        ...
                        <select class="select @error('domaine') is-invalid @enderror" name="domaine" aria-describedby="domaine" placeholder="Sélectionner le domaine" required>
                            @foreach ($typeParametre->where('code_typ', 'dom')->first()->parametres as $parametre)
                                <option value="{{ $parametre->id_par }}" {{ $projet->iddom_prj == $parametre->id_par ? 'selected' : '' }}>{{ $parametre->libelle_par }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="col-md-12">
                    <div class="form-group mb-3">
                        ...
                        <input type="text" class="form-control @error('nom') is-invalid @enderror" id="nom" name="nom" aria-describedby="nom" placeholder="Saississez le nom" value="{{ $projet->libelle_prj }}" required>
                    </div>
                </div>
                ...
                <div class="col-md-6">
                    <div class="form-group mb-3">
                        ...
                        <input type="number" class="form-control @error('budget') is-invalid @enderror" id="budget" name="budget" aria-describedby="budget" placeholder="Saississez le budget" value="{{ $projet->budget_recherche_prj }}">
                    </div>
                </div>
                ...
                <div class="col-md-6">
                    <div class="form-group mb-3">
                        ...
                        <input type="number" class="form-control @error('investissement_min') is-invalid @enderror" id="investissement_min" name="investissement_min" aria-describedby="investissement_min" placeholder="Saississez le budget minimum" value="{{ $projet->investissement_min_prj }}">
                    </div>
                </div>
                ...
                <div class="col-md-12">
                    <div class="form-group mb-3">
                        ...
                        <textarea type="text" class="summernote" id="summernote" name="description" aria-describedby="description">{{ $projet->description_prj }}</textarea>
                    </div>
                </div>
